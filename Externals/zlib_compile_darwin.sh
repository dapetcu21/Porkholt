...
for ARCH in $IOSARCHS; do
    export CFLAGS="-arch $ARCH -isysroot $IOSSDK -miphoneos-version-min=4.2 -pipe -no-cpp-precomp"
...
for ARCH in $SIMARCHS; do
    export CFLAGS="-arch $ARCH -isysroot $SIMSDK -miphoneos-version-min=4.2 -pipe -no-cpp-precomp"
    make distclean 2>&1 > /dev/null
    ./configure && make || exit 1
