...
-(void)addURL:(NSURL*)url atIndexPath:(NSIndexPath*)indexPath
{
	...
	if ([extension isEqualToString:@"png"])
		fs.antiAliasing = [fman fileExistsAtPath:[path stringByAppendingPathExtension:@"aa"]];
	...
	if (isDir)
	{
		...
		for (NSString * fpath in dirs)
		{
			...
			if ([ext isEqualToString:@"aa"]) continue;
...
-(void)reload:(id)sender
{
    ...
    {
        [reloadButton highlight:YES];
        [reloadButton performSelector:@selector(highlight:) withObject:nil afterDelay:0.1];
    }
}
