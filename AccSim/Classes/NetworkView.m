...
- (void)sendPacket:(const char*)msg length:(size_t)len
{
	// only process is network is enabled and socket initialized OK
	if( networkEnabled && udpSocket != -1 )
	{
		int error = sendto(udpSocket, msg, len, 0, (struct sockaddr*)&targetAddress, sizeof(targetAddress));
		if( error < 0 )
		{
...
- (void)accelerationUpdate:(NSNotification*)notification {
	...
	[self sendPacket:msg length:strlen(msg)];
}
