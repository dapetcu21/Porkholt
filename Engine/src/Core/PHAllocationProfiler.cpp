...
void PHAllocationProfiler::Object::addTrace(PHAllocationProfiler::Object::Trace * t)
{
    ...
    if (t->type != Trace::TraceAlloc)
        if (name == "")
            name = typeid(*static_cast<PHObject*>(address)).name();
    switch (t->type)
    { 
...
PHAllocationProfiler::Object::Trace::Trace(void * addr, enum PHAllocationProfiler::Object::Trace::trace_type _type, int _refcount, int strip, int count) : refcount(_refcount), address(addr), type(_type)
{
    strip++;
